...
def outputs():
    ...
    questions = [
        ...
        (f"What does Finder integration add in v{current}?", f"The v{current} current source adds a Finder Sync extension with right-click Cut, Move Here, and Cancel Cut, a toolbar menu, and pending-cut badges. Enable it from Cut & Move and choose Show Finder Actions In to manage its folders. The main app performs menu-driven moves without automatic overwrites. These features require the current source version; check the published download version before installing."),
...
Current-source enhancements include a native Finder Sync extension, right-click Cut and Move Here actions, a toolbar menu, pending-cut badges, configurable folders, shared keyboard/menu cut state, and collision checks for menu-driven moves. If the current source differs from the published version, treat these as preview features, not download guarantees.
'''
