...
- Automatic release updates: `.github/workflows/release-marketing.yml` runs when a stable release is published. It reads the latest stable release, requires uploaded DMG, ZIP, checksums, and license assets, updates `marketing.json`, regenerates the page/README/metadata/FAQs/share copy, tests them, commits to `main`, and explicitly requests a Pages build. It then checks the live download link. Existing signing/notarization verification remains the responsibility of `make release` and `make publish`; asset presence alone does not verify signatures.
- Push source changes to `main` before publishing the matching release. Draft releases, prereleases, and tag-only pushes do not promote downloads. Old events cannot downgrade the recorded version. New release highlights should be written in the page and README as part of the source changes; the workflow synchronizes versions and availability, not prose generation.
- Recovery: run `gh workflow run release-marketing.yml` to repeat synchronization safely after correcting missing assets or a failed build. It is idempotent and creates no commit when already current. If a future workflow publishes with `GITHUB_TOKEN`, explicitly dispatch this workflow afterward because token-generated release events do not trigger other workflows. No personal access token is required for this automation.
