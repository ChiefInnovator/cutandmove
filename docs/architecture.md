...
Unit tests exercise event pairs, modifiers, repeats, cancellation, clipboard guards, focus classification, monitoring decisions, Unicode remapping, login-item outcomes, menu snapshots, app-group storage, and safe file moves. Existing UI tests exercise launch behavior. The opt-in `make test-finder` harness checks keyboard moves and ordinary copies in real Finder views. Native toolbar Cut and Move Here have also been smoke-tested with source removal and content preservation verified. Context-menu and mixed keyboard/menu paths still need complete interactive coverage; unit tests do not replace that.
